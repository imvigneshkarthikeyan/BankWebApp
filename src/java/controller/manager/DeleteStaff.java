...
public class DeleteStaff implements Action, ServletRequestAware,ServletResponseAware {
...
//                    int managerBankID = StaffDAO.getStaffBankID((String) request.getSession().getAttribute("currentUser"));
                if (utilities.Validator.deleteStaffValidator(jsonObj, StaffDAO.getStaffBankID((String) request.getSession().getAttribute("currentUser")), response)) {
                    boolean resultOfDeleteStaff = StaffDAO.deleteStaff(jsonObj.getString("userID"));
                    if (!resultOfDeleteStaff) {
                        utilities.Util.setStatusCodeMessage(400, "errorMessage", "Error occured, Please try again.", response);
                    } else {
                        utilities.Util.setStatusCodeMessage(200, "successMessage", "Staff deleted successfully!", response);
                    }
                }
